...
(function() {
  var add_list, choose_name, choosing_timeout, delay, display_person, group, highlight_choice, index, limit, next_person, remove_list, shuffled_people, tick, _ref;

  group = (_ref = window.MH) != null ? _ref.group : void 0;
  ...
  choose_name = function(results_index) {
    var results_el;
    delay = 1;
    ...
    return tick(results_el, function() {
      return setTimeout(function() {
        if (results_index + 1 < group.lists.length) {
          return choose_name(results_index + 1);
        }
      }, 500);
    });
  };

  tick = function(results_el, callback) {
    ...
    return choosing_timeout = setTimeout(function() {
      return tick(results_el, callback);
    }, delay);
  };

  highlight_choice = function(results_el) {
    return $('.person.displayed', results_el).addClass('choosen');
  };
  ...
  next_person = function() {
    ...
    return shuffled_people[index];
  };

  display_person = function(results_el, person) {
    $('.person.displayed', results_el).removeClass('displayed');
    return $(".person-" + person.id, results_el).addClass('displayed');
  };

  if ((group != null ? group.lists.length : void 0) === 0) {
    $('#chooser').addClass('disabled').attr({
      'disabled': true
    });
  }
  ...
  add_list = function() {
    ...
    return $('.lists-container td').attr('width', "" + (100 / index) + "%");
  };

  remove_list = function() {
    return $('.lists-container td:last').remove();
  };
